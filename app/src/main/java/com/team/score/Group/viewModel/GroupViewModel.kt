...
class GroupViewModel: ViewModel() {
    ...
    // 피드 삭제
    fun deleteFeed(activity: MainActivity, feedId: Int, onSuccess: () -> Unit) {
        val apiClient = ApiClient(activity)
        val tokenManager = TokenManager(activity)

        apiClient.apiService.deleteFeed(tokenManager.getAccessToken().toString(), feedId).enqueue(object :
            Callback<String?> {
            override fun onResponse(
                call: Call<String?>,
                response: Response<String?>
    ...
    // 메이트 차단
    fun blockMate(activity: MainActivity, mateId: Int) {
        val apiClient = ApiClient(activity)
        val tokenManager = TokenManager(activity)

        apiClient.apiService.blockMate(tokenManager.getAccessToken().toString(), tokenManager.getUserId(), mateId).enqueue(object :
            Callback<String?> {
            override fun onResponse(
                call: Call<String?>,
                response: Response<String?>
    ...
    // 메이트 신고
    fun reportMate(activity: MainActivity, mateId: Int, reason: String?, comment: String?) {
        val apiClient = ApiClient(activity)
        val tokenManager = TokenManager(activity)

        var body = ReportMateRequest(tokenManager.getUserId(), mateId, reason ?: "", comment ?: "")

        apiClient.apiService.reportMate(tokenManager.getAccessToken().toString(), body).enqueue(object :
            Callback<String?> {
            override fun onResponse(
                call: Call<String?>,
                response: Response<String?>
